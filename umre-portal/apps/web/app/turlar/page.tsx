...
async function getTours(searchParams: any) {
    ...
    try {
        const res = await fetch(`http://localhost:4000/api/tours?${params.toString()}`, {
            cache: 'no-store'
        });
        if (res.ok) {
...
async function getTourTypes() {
    try {
        const res = await fetch('http://localhost:4000/api/tour-types', { cache: 'no-store' });
...
async function getLocations() {
    try {
        const res = await fetch('http://localhost:4000/api/locations', { cache: 'no-store' });
        if (res.ok) return await res.json();
    } catch (err) {
        console.error(err);
    }
